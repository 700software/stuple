[**stuple**](README.md)

***

# stuple

## Type Aliases

- [ObjectNotArray](type-aliases/ObjectNotArray.md)
- [SetState](type-aliases/SetState.md)
- [SetStateSimple](type-aliases/SetStateSimple.md)
- [SimpleState](type-aliases/SimpleState.md)
- [SimpleStuple](type-aliases/SimpleStuple.md)
- [Stuple](type-aliases/Stuple.md)
- [UsedState](type-aliases/UsedState.md)

## Functions

- [asStuple](functions/asStuple.md)
- [subSetState](functions/subSetState.md)
- [subState](functions/subState.md)
- [subStuple](functions/subStuple.md)
- [useRerenderTrigger](functions/useRerenderTrigger.md)
- [useStateWithDeps](functions/useStateWithDeps.md)
- [useStuple](functions/useStuple.md)
- [useStupleWithDeps](functions/useStupleWithDeps.md)
